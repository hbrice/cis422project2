		...
		for day in daysNeedingCoverage:
			...
			for hour in range(day.start_time.hour, day.end_time.hour):
				...
				for e in Availibity.objects.filter(AvailibleDay=day.AvailibleDay):
					if e.start_time.hour<= hour and hour < e.end_time.hour and not User.objects.get(username=e.user).isManager:
				...
				if len(tmpSet) == 0:
					...
					newSchedule = Schedule(user=Employee.objects.get(username=poolOfEmployees[0].user),AvailibleDay=day.AvailibleDay,start_time=newStart,end_time=newEnd)
					newSchedule.save()
